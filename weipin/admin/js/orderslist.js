$(document).ready(function() {
    $("#selArea").change(function() { if($(this).val()!="") { $.cookie("OrdersAreaID",$(this).val(),{ expires: 1000 }); } else { $.cookie("OrdersAreaID",null); } $.cookie("OrdersDeliveryPeriod",$("#selDeliveryPeriod").val(),{ expires: 1000 });location.href="OrdersList.aspx?p=1&aid="+$(this).val()+"&dp="+$("#selDeliveryPeriod").val(); });
    ...
    $("#selDeliveryPeriod").change(function() { $.cookie("OrdersDeliveryPeriod",$(this).val(),{ expires: 1000 });location.href="OrdersList.aspx?p=1&aid="+$("#selArea").val()+"&dp="+$(this).val(); });
    //送货时间
...
function OutLibrary(_oid,_tdid) {
    if($("#sel"+_oid).val()!="") {
        ...
        $.ajax({ type: "post",url: "ajaxservice/Orders.aspx",async: true,data: { ptype: "UpdateOrdersOutLibraryByOrderID",oid: _oid,cid: _cid,sql: _sql,gidarr: _gidarr.substring(1) },
