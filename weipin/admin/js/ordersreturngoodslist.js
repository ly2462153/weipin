...
$("#selArea").change(function() { if($(this).val()!="") { $.cookie("OrdersAreaID",$(this).val(),{ expires: 1000 }); } else { $.cookie("OrdersAreaID",null); } $.cookie("OrdersDeliveryPeriod",$("#selDeliveryPeriod").val(),{ expires: 1000 });location.href="OrdersReturnGoodsList.aspx?p=1&aid="+$(this).val()+"&dp="+$("#selDeliveryPeriod").val(); });
...
$("#selDeliveryPeriod").change(function() { $.cookie("OrdersDeliveryPeriod",$(this).val(),{ expires: 1000 });location.href="OrdersReturnGoodsList.aspx?p=1&aid="+$("#selArea").val()+"&dp="+$(this).val(); });
//加载商品交易状态
...
//取货
function ExtractGoods(_oid,_tdid) {
if($("#sel"+_oid).val()!="") {
$("#"+_tdid).text("loading");
var _cid=$("#sel"+_oid).val();
var _sql="";
$("td[name='tdOID"+_oid+"']").each(function() {
//            var _ordersgoodsid=$(this).children("input").val();待删除2012-6-7
...
//            if(_hidgoodsstatus=="4") {
//                _sql+=" update Goods set Inventory=Inventory+"+_count+" where GoodsID="+_gid;
//            }
});
$.ajax({ type: "post",url: "ajaxservice/Orders.aspx",async: true,data: { ptype: "UpdateOrdersAndGoodsExtractGoodsByOrderID",oid: _oid,cid: _cid },
