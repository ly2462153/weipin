$(document).ready(function() { CreateBrowser($("#divUploadBox")); });
//创建上传控件
function CreateBrowser(obj) {
$.ajax({ type: "get",url: "/xml/goods/goodspaths/"+$("#hidGoodsID").val()+".xml",dataType: "xml",error: function() { alert("服务器响应超时，数据加载失败！"); },
...
_index=parseInt(i)+1;
var _path=$(this).attr("value");
strContent+="<div class=\"object\"><div id=\"divShowPic"+_index+"\"><img src='"+_path+"' width='172' height='172'/></div><div id=\"divUploading"+_index+"\" class=\"prompt\">文件上传成功!<a href='#' onclick=\"DeletePicture("+_index+",'"+_path+"');return false;\">[删除]</a></div></div>";
_arrindex+="|"+_index;
_jointpath+="|"+_path;
if(i==0) { $("#hidMainPicPath").val(_path); }
});
$("#hidOldPicPath").val(_jointpath.substring(1));
$("#hidPicPath").val(_jointpath.substring(1));
$("#hidControlUpload").val(_arrindex.substring(1));
for(var i=parseInt(_index)+1;i<=5;i++) {
strContent+="<div class=\"object\"><div id=\"divShowPic"+i+"\"><img src='img/fork.jpg'/></div><div id=\"divUploading"+i+"\" class=\"prompt\"></div></div>";
}
...
var _relativepath=$(this).attr("src").substring($(this).attr("src").indexOf("/file/images/"));
_path+="|"+_relativepath;
}
});
