...
{
    public partial class OrdersChangeGoodsList : Common.BasePageAdmin
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                BindOrdersChangeGoodsList(Convert.ToInt32(Request.QueryString["aid"].ToString()), Convert.ToByte(Request.QueryString["dp"].ToString()));
            ...
            else
            {
                ...
                {
                    BindOrdersChangeGoodsList(Convert.ToInt32(Request.Cookies["OrdersAreaID"].Value), Convert.ToByte(Request.Cookies["OrdersDeliveryPeriod"].Value));
        ...
        private void BindOrdersCountStatistics()
        {
            ...
            List<Model.Orders> lmo = bos.SelectOrdersCountStatisticsByOrdersStatus(3);
        ...
        /// </summary>
        private void BindOrdersChangeGoodsList(int _aid, byte _deliveryperiod)
        {
            ...
            DataList<Model.Orders> dmo = bo.SelectOrdersChangeGoodsOfTop(_ordercount, _aid, _deliveryperiod);
            if (dmo != null && dmo.Rows.Count > 0)
            {
                spDataCount.InnerText = dmo.Total.ToString();
                rptOrdersChangeGoodsList.DataSource = dmo.Rows;
                rptOrdersChangeGoodsList.DataBind();
            }
            else
            {
                divShowDataCount.Visible = false;
        ...
        /// <param name="e"></param>
        protected void rptOrdersChangeGoodsList_PreRender(object sender, EventArgs e)
        {
            MergeRowsRPT(rptOrdersChangeGoodsList);
        }
        /// <summary>
        /// 合并Repeater相同数据行
