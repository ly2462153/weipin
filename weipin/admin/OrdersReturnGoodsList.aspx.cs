...
{
    public partial class OrdersReturnGoodsList : Common.BasePageAdmin
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                BindOrdersReturnGoodsList(Convert.ToInt32(Request.QueryString["aid"].ToString()), Convert.ToByte(Request.QueryString["dp"].ToString()));
            ...
            else
            {
                ...
                {
                    BindOrdersReturnGoodsList(Convert.ToInt32(Request.Cookies["OrdersAreaID"].Value), Convert.ToByte(Request.Cookies["OrdersDeliveryPeriod"].Value));
        ...
        private void BindOrdersCountStatistics()
        {
            ...
            List<Model.Orders> lmo = bos.SelectOrdersCountStatisticsByOrdersStatus(4);
        ...
        private void BindAreas(string aid)
        {
            ...
            else
            {
                XmlDocument doc = new XmlDocument();
                doc.Load(Server.MapPath("~/xml/areas/areas.xml"));
                XmlNodeList nodelist = doc.GetElementsByTagName("province");
                if (nodelist.Count > 0)
                {
                    string _str = string.Empty;
                    selProvince.Items.Add(new ListItem("请选择", ""));
        ...
        /// </summary>
        private void BindOrdersReturnGoodsList(int _aid, byte _deliveryperiod)
        {
            ...
            DataList<Model.Orders> dmo = bo.SelectOrdersReturnGoodsOfTop(_ordercount, _aid, _deliveryperiod);
            if (dmo != null && dmo.Rows.Count > 0)
            {
                spDataCount.InnerText = dmo.Total.ToString();
                rptOrdersReturnGoodsList.DataSource = dmo.Rows;
                rptOrdersReturnGoodsList.DataBind();
            }
            else
            {
                divShowDataCount.Visible = false;
        ...
        /// <param name="e"></param>
        protected void rptOrdersReturnGoodsList_PreRender(object sender, EventArgs e)
        {
            MergeRowsRPT(rptOrdersReturnGoodsList);
        }
        /// <summary>
        /// 合并Repeater相同数据行
        ...
        public static void MergeRowsRPT(Repeater rpt)
        {
            for (int rowIndex = rpt.Items.Count - 2; rowIndex >= 0; rowIndex--)
            {
                ...
                if (cells.InnerText == previouscells.InnerText)
                {
                    ...
                    //合并ShippingAddress列
                    HtmlTableCell cellsShippingAddress = rpt.Items[rowIndex].FindControl("ShippingAddress") as HtmlTableCell;
                    HtmlTableCell previouscellsShippingAddress = rpt.Items[rowIndex + 1].FindControl("ShippingAddress") as HtmlTableCell;
                    if (cellsShippingAddress.InnerText == previouscellsShippingAddress.InnerText)
                    {
                        cellsShippingAddress.RowSpan = previouscellsShippingAddress.RowSpan < 2 ? 2 : previouscellsShippingAddress.RowSpan + 1;
                        previouscellsShippingAddress.Visible = false;
                    }
                }
                //合并tdCourierID列
                ...
                //合并tdExtractGoods列
                HtmlTableCell cellsName = rpt.Items[rowIndex].FindControl("tdExtractGoods") as HtmlTableCell;
                HtmlTableCell previouscellsName = rpt.Items[rowIndex + 1].FindControl("tdExtractGoods") as HtmlTableCell;
