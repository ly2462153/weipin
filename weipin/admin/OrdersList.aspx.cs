...
{
    public partial class OrdersList : Common.BasePageAdmin
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                BindOrdersList(Convert.ToInt32(Request.QueryString["aid"].ToString()), Convert.ToByte(Request.QueryString["dp"].ToString()));
            ...
            else
            {
                ...
                {
                    BindOrdersList(Convert.ToInt32(Request.Cookies["OrdersAreaID"].Value), Convert.ToByte(Request.Cookies["OrdersDeliveryPeriod"].Value));
        ...
        private void BindOrdersCountStatistics()
        {
            ...
            List<Model.Orders> lmo = bos.SelectOrdersCountStatisticsByOrdersStatus(1);
        ...
        /// </summary>
        private void BindOrdersList(int _aid, byte _deliveryperiod)
        {
            ...
            DataList<Model.Orders> dmo = bo.SelectOrdersOfTop(_ordercount, _aid, _deliveryperiod);
            if (dmo != null && dmo.Rows.Count > 0)
            {
                spDataCount.InnerText = dmo.Total.ToString();
                rptOrdersList.DataSource = dmo.Rows;
                rptOrdersList.DataBind();
            }
            else
            {
                divShowDataCount.Visible = false;
        ...
        /// <param name="e"></param>
        protected void rptOrdersList_PreRender(object sender, EventArgs e)
        {
            MergeRowsRPT(rptOrdersList);
        }
        /// <summary>
        /// 合并Repeater相同数据行
